...
# Modern AI-Powered Search System using OpenAI GPT-4
class ModernAISearchEngine:
    ...
    def _build_column_metadata(self) -> Dict[str, Dict]:
        """Build comprehensive metadata for each column."""
        metadata = {}
        for col in self.df.columns:
            col_lower = col.lower()
            
            # Determine column type and category
            if pd.api.types.is_numeric_dtype(self.df[col]):
                col_type = "numeric"
                sample_values = [float(self.df[col].min()), float(self.df[col].max())]
            elif pd.api.types.is_datetime64_any_dtype(self.df[col]):
                col_type = "datetime"
                sample_values = [str(self.df[col].min()), str(self.df[col].max())]
            else:
            ...
            metadata[col] = {
                ...
                'description': f"{category} - {col_type} data"
            }
        
        return metadata
    
    def _generate_ai_search_query(self, user_query: str) -> Dict:
        """Use OpenAI GPT-4 to convert natural language to structured search criteria."""
        if not self.client:
        ...
        try:
            ...
            prompt = f"""
You are an expert medical data analyst. Convert the user's natural language query into structured search criteria for a biobanking database.
...
{{
    ...
    "confidence": 0.95,
    "suggestions": ["alternative search terms or clarifications"]
}}

Focus on medical terminology, patient demographics, and clinical conditions. Be precise and comprehensive.
"""
            
            response = self.client.chat.completions.create(
                model="gpt-4",
                messages=[
                    {"role": "system", "content": "You are a medical data search expert. Always respond with valid JSON."},
            ...
            search_criteria = json.loads(ai_response)
            
            return search_criteria
            
        except Exception as e:
            st.warning(f"AI search failed: {str(e)}. Using fallback search.")
            return self._fallback_search(user_query)
    ...
    def _build_ai_context(self) -> str:
        ...
        for category, columns in categories.items():
            ...
            for col_info in columns[:10]:  # Limit to first 10 columns per category
                context_parts.append(f"  - {col_info['name']} ({col_info['type']})")
                if col_info['samples']:
                    context_parts.append(f"    Sample values: {col_info['samples']}")
    ...
    def _fallback_search(self, query: str) -> Dict:
        ...
        # Simple medical term mapping
        medical_terms = {
            'diabetes': ['Hx_Diabetes', 'diabetes'],
            'hypertension': ['Hx_Hypertension', 'hypertension'],
            'heart': ['Hx_Heart_Disease', 'heart'],
            'vision': ['Sx_Vision', 'vision'],
            'pain': ['Sx_Pain', 'pain'],
            'male': ['GENDER', 'male'],
        ...
        for term, related_cols in medical_terms.items():
                ...
                for col in related_cols:
                    if col in self.df.columns:
                        ...
                        conditions.append({
                            ...
                            "operator": "contains",
                            "value": term,
                            "description": f"Contains '{term}'"
                        })
        
        return {
            "interpretation": f"Basic keyword search for: {query}",
            "search_criteria": {
                "columns": columns,
            ...
            "confidence": 0.6,
            "suggestions": ["Enable OpenAI API for better search results"]
        }
    
    def search(self, query: str) -> Tuple[pd.DataFrame, Dict]:
        """Perform modern AI-powered search."""
        if not query.strip():
            return self.df, {}
        
        # Get AI interpretation
        search_criteria = self._generate_ai_search_query(query)
        
        # Apply search criteria
        filtered_df = self._apply_search_criteria(search_criteria)
        
        return filtered_df, search_criteria
    
    def _apply_search_criteria(self, search_criteria: Dict) -> pd.DataFrame:
        """Apply AI-generated search criteria to the dataframe."""
        if not search_criteria.get('search_criteria', {}).get('conditions'):
            return self.df
        
        conditions = search_criteria['search_criteria']['conditions']
        logical_op = search_criteria['search_criteria'].get('logical_operator', 'AND')
    ...
    def get_search_suggestions(self, partial_query: str) -> List[str]:
        ...
        try:
            prompt = f"""
Given this partial query: "{partial_query}"

Suggest 5 natural language search queries for a medical biobanking database. 
Focus on medical conditions, demographics, symptoms, and medications.
...
"""
            ...
            response = self.client.chat.completions.create(
                model="gpt-4",
                messages=[
                    {"role": "system", "content": "You are a medical search assistant. Return only valid JSON arrays."},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.3,
                max_tokens=200
            ...
            return [
                f"{partial_query} patients",
                f"{partial_query} symptoms",
                f"{partial_query} medication",
                f"{partial_query} history"
            ]

# Initialize modern AI search engine
@st.cache_resource
def get_modern_search_engine(df: pd.DataFrame) -> ModernAISearchEngine:
    return ModernAISearchEngine(df)

search_engine = get_modern_search_engine(data)
...
with data_tab:
    ...
    with col1:
        search_query = st.text_input(
            ...
            help="Supports natural language queries, fuzzy matching, and semantic search. Handles typos and medical terminology."
        )
    ...
    with col2:
        search_type = st.selectbox(
            ...
            help="Smart: Combines all methods. Exact: Precise matching. Fuzzy: Typo-tolerant. Semantic: Meaning-based."
        )
    ...
    if search_query:
        with st.spinner("🔍 Searching with AI..."):
            filtered_df, search_criteria = search_engine.search(search_query)
        
        # Show search results summary
        st.success(f"✅ Found {len(filtered_df)} results for '{search_query}' using AI search")
        
        # Show search explanation
        with st.expander("🔍 How the AI interpreted your search", expanded=False):
            st.write("**Search Analysis:**")
            if search_criteria.get('interpretation'):
                st.write(f"• {search_criteria['interpretation']}")
            
            if search_criteria.get('search_criteria', {}).get('conditions'):
                for cond in search_criteria['search_criteria']['conditions']:
                    if cond['description']:
                        st.write(f"• {cond['description']}")
            
            st.write(f"**Search Method:** AI-powered search with semantic understanding")
    else:
